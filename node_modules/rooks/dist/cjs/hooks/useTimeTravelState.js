...
function useTimeTravelState(initialValue) {
     ...
     */
    const reset = react.useCallback((resetInitialValue) => {
    ...
    const updateValue = react.useCallback((val, options) => {
    ...
    const goForwardInternal = react.useCallback((step = 1) => {
        setHistory((currentHistory) => {
    ...
    const goBackwardInternal = react.useCallback((step = -1) => {
        setHistory((currentHistory) => {
            const { future, past, present } = currentHistory;
            if (past.length === 0) {
                return currentHistory;
    ...
    const go = react.useCallback((step) => {
        const stepNum = utils.isNumber(step) ? step : Number(step);
        if (stepNum === 0) {
            return;
        }
        if (stepNum > 0) {
...
exports.useTimeTravelState = useTimeTravelState;
